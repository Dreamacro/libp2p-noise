...
func makeTransport(b *testing.B, psk []byte) *Transport {
	b.Helper()
...
func setupEnv(b *testing.B) *benchenv {
	...
	initTpt := makeTransport(b, psk)
	respTpt := makeTransport(b, psk)
...
func benchDataTransfer(b *benchenv, dataSize int64, m testMode) {
	...
	for i := 0; i < len(plainTextBufs); i++ {
		...
		case readBufferGtEncMsg:
			rbuf = make([]byte, len(plainTextBufs[i])+Overhead+1)
