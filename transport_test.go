...
func newTestTransport(t *testing.T, typ, bits int, psk []byte) *Transport {
...
func newTestTransportWithMuxers(t *testing.T, typ, bits int, psk []byte, muxers []protocol.ID) *Transport {
	transport := newTestTransport(t, typ, bits, psk)
...
func TestDeadlines(t *testing.T) {
	...
	initTransport := newTestTransport(t, crypto.Ed25519, 2048, psk)
	respTransport := newTestTransport(t, crypto.Ed25519, 2048, psk)
...
func TestBufferEqEncPayload(t *testing.T) {
	...
	after := make([]byte, len(before)+Overhead)
...
func TestEarlyDataAccepted(t *testing.T) {
	handshake := func(t *testing.T, client, server EarlyDataHandler) {
		...
		initTransport, err := newTestTransport(t, crypto.Ed25519, 2048, psk).WithSessionOptions(EarlyData(client, nil))
		require.NoError(t, err)
		tpt := newTestTransport(t, crypto.Ed25519, 2048, psk)
...
func TestEarlyfffDataAcceptedWithNoHandler(t *testing.T) {
	...
	initTransport, err := newTestTransport(t, crypto.Ed25519, 2048, psk).WithSessionOptions(EarlyData(clientEDH, nil))
	require.NoError(t, err)
	respTransport := newTestTransport(t, crypto.Ed25519, 2048, psk)
...
func TestHandshakeWithTransportEarlyData(t *testing.T) {
	...
	noiseHandshake := func(t *testing.T, initProtos, respProtos []protocol.ID, expectedProto protocol.ID) {
		psk := newPsk()
		initTransport := newTestTransportWithMuxers(t, crypto.Ed25519, 2048, psk, initProtos)
		respTransport := newTestTransportWithMuxers(t, crypto.Ed25519, 2048, psk, respProtos)
