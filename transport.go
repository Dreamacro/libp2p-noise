...
const ID = "/noise/1.0/Noise_XXpsk3_DH25519_AESGCM_BLAKE2b"
...
type FactoryFunc = func(id protocol.ID, privkey crypto.PrivKey, muxers []tptu.StreamMuxer) (*Transport, error)

// New creates a new Noise transport using the given private key and psk as its
// libp2p identity key.
func New(psk []byte) FactoryFunc {
	return func(id protocol.ID, privkey crypto.PrivKey, muxers []tptu.StreamMuxer) (*Transport, error) {
