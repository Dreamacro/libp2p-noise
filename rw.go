...
// inclusive of the MAC size (16 bytes, GCM for noise-libp2p).
const MaxTransportMsgLength = 0xffff

const Overhead = 16 // gcmTagSize
...
const MaxPlaintextLength = MaxTransportMsgLength - Overhead // gcmTagSize
...
func (s *secureSession) Write(data []byte) (int, error) {
	...
	if total < MaxPlaintextLength {
		cbuf = pool.Get(total + Overhead + LengthPrefixLength)
	} else {
